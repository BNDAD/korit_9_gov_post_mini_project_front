...
function Home() {
  ...
  console.log(isFetching); //요청만 날라감
  console.log(isLoading); //최초 렌더링
    ...
    <div css={s.layout}>
      <div css={s.feedContainer(commentOpen)}>
            ...
            feeds.data.contents.map((feed) => (
              <div key={feed.feedId} css={s.feedItemContainer}>
                ...
                <main>
                    ...
                    <div css={s.feedImageContainer}>
                      ...
                      >
                        {feed.imageFiles.map((file) => (
                          <div
                            css={s.feedImage(
                              "http://localhost:8080/image" + file.filePath
                            )}
                          ></div>
                        ))}
                      </Slider>
                  ...
                  <div css={s.feedContentContainer}>{feed.content}</div>
                </main>
                <footer>
                  <div>{false ? <IoMdHeart /> : <IoMdHeartEmpty />}</div>
                  <div onClick={handleCommentOnClick}>
                    <IoChatbubbleOutline />
                  </div>
                </footer>
              </div>
        ...
        <div ref={loadMoreRef} style={{ padding: "10px 0" }}>
          {isFetching && !isLoading && <PulseLoader />}
        </div>
      </div>
      <div css={s.commentContainer(commentOpen)}></div>
